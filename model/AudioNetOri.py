...
class AudioNetOri(nn.Module):
    ...
    def __init__(self, num_class, transform_layer=None, transform_param=None):
        ...
        # [need change]
        self.drop = MyDropout(inplace=True, indices=[0], p=0)
    ...
    def encode_feat(self, x):
        ...
        x, _ = x.max(2)
        '''
        feature_flag = np.load('feature_flag.npy')[-1]
        
        if feature_flag == 0:
            feature_array = np.load('feature_array.npy')
            
            if feature_array.shape[0] > 500000:
                feature_flag = 1
                arr = np.array([feature_flag])
                np.save("feature_flag.npy", arr)
                
            a = np.array(x.clone().detach().cpu())
            feature_array = np.vstack((feature_array, a))
            np.save('feature_array.npy', feature_array)
        '''    
        
        '''
        if os.path.exists('feature_array.npy'):
            array_list = np.load('feature_array.npy', allow_pickle=True)
        else:
            array_list = []
        if array_list:
            previous_array = array_list
            concatenated_array = torch.cat((previous_array, x.clone().detach()), dim=0)
        else:
            concatenated_array = x
        
        array_list.append(concatenated_array)
        np.save('feature_array.npy', array_list)
        '''
        ...
        '''
        attack_flag = np.load('attack_flag.npy')[-1]
        drop_neuro_num = np.load('drop_neuro_num.npy')[-1]
        if attack_flag == 1: 
            print("剪枝特征！\r", end='')
